...
- 互联网有太多的服务需要使用证书来验证身份，以至于客户端（操作系统或浏览器等）无法内置所有证书，需要通过服务端将证书发送给客户端。**客户端内置的是 CA 的根证书(Root Certificate)，HTTPS 协议中服务器会发送证书链（Certificate Chain）给客户端。**客户端内置CA的根证书(Root Certificate)（浏览器、操作系统里）， 里面包含`CA公钥`。**客户端（浏览器）的"证书管理器"，有"受信任的根证书颁发机构"列表。客户端会根据这张列表，查看解开数字证书的公钥是否在列表之内。**
- 服务器去CA申请一个证书，**CA用私钥对服务器公钥等信息进行加密生成了证书**，然后发送给服务器。

- 为了确保根证书的绝对安全性，所以有了**[证书链](https://blog.csdn.net/huzhenv5/article/details/104578198)**，所以CA不会直接颁布证书，而是通过intermediates certificates给网站颁发证书所谓证书链的验证，是想通过证书链追溯到可信赖的CA的根（ROOT）。换句话说，要验证签发用户实体证书的CA是否是权威可信的CA，如CFCA。**证书链验证的要求是，路径中每个证书从最终实体到根证书都是有效的，并且每个证书都要正确地对应发行该证书的权威可信任性CA。**
...
**本地dns服务端-外网：这部分属于迭代查询。**
 ...
 下面举一个例子演示整个查询过程，在查询前会先在缓存里找，找不到再查询。
...
### 5.[GET和POST是HTTP请求的两种基本方法，区别](https://blog.csdn.net/ever_siyan/article/details/87935455) 
...
### 6.代理服务器的工作原理是怎样的？[代理和网关](https://blog.csdn.net/sjailjq/article/details/82287345)

### 7.[cookie和session](https://www.cnblogs.com/l199616j/p/11195667.html)
...
### 进程同步，控制多个进程按一定顺序执行：信号量、临界区、同步与互斥、管程（重点）
...
### [HashMap](https://blog.csdn.net/belongtocode/article/details/100887653)（会问）

综合了数组和链表的优点，底层就是数组和链表实现的，JDK1.8后使用红黑树实现
...
## [智力题](https://blog.csdn.net/qq_40058686/article/details/104664136)（略过）
...
8. 100只兔子，重量不同。取出10只，某一只的体重最重。问这只兔子在全部兔子体重排名中的期望值。

题目等价于100个兔子编号为1，2，……100，随机选十只，求其中编号最小的期望值。选的十只外有九十只兔子，相当于用十只兔子将九十只兔子分成了11份，其中每份的地位相同，且11个数字和选的十只兔子的方法一一对应。第一堆兔子的期望数是90/11，第一只兔子的期望是90/11+1=101/11

基本逻辑是兔子依次编号成1-100 重量依次递减 问10只兔子编号最小的那个兔子编号的期望值

其实就是求出i号兔子作为选中的10只兔子编号最小的概率choose(100-i, 9)/choose(100,10) 乘以i累和用组合数的一些性质和恒等式 求了一下 大概是 101/11

9. 50黑50红两个桶怎么使抽到红球概率更大 
